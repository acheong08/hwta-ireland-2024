...
def solution_data_preparation(solution, servers, datacenters, selling_prices):
    ...
    solution = solution.merge(
        selling_prices, on=["server_generation", "latency_sensitivity"], how="left"
    )
...
def check_actions(solution):
    ...
    if not (solution[solution["time_step"] == 1]["action"] == "buy").all():
        raise (
            ValueError('At time-step 1 it is only possible to use the "buy" action.')
        )
    return solution.reset_index(drop=True, inplace=False)
...
def drop_duplicate_server_ids(solution):
    ...
    drop = solution[
        (solution["server_id"].duplicated()) & (solution["action"] == "buy")
    ].index
    if drop.any():
...
def change_selling_prices_format(selling_prices):
    ...
    # REST OF CODE
    selling_prices = selling_prices.pivot(
        index="server_generation", columns="latency_sensitivity"
    )
    selling_prices.columns = selling_prices.columns.droplevel(0)
    return selling_prices


def get_actual_demand(demand: list[Any]) -> pd.DataFrame:
    ...
    actual_demand = actual_demand.pivot(
        index=["time_step", "server_generation"], columns="latency_sensitivity"
    )
    actual_demand.columns = actual_demand.columns.droplevel(0)
    actual_demand = actual_demand.loc[
        actual_demand[get_known("latency_sensitivity")].sum(axis=1) > 0
    ]
    actual_demand = actual_demand.reset_index(
        ["time_step", "server_generation"], col_level=1, inplace=False
    )
...
def get_capacity_by_server_generation_latency_sensitivity(fleet):
    ...
    Z = (
        fleet.groupby(by=["server_generation", "latency_sensitivity"])["capacity"]
        .sum()
        .unstack()
    )
...
def check_datacenter_slots_size_constraint(fleet):
    ...
    slots = fleet.groupby(by=["datacenter_id"]).agg(
        {"slots_size": "sum", "slots_capacity": "mean"}
    )
...
def update_fleet(ts, fleet, solution):
    if fleet.empty:
        ...
        server_id_action = (
            solution[["action", "server_id"]]
            .groupby("action")["server_id"]
            .apply(list)
            .to_dict()
        )
        # BUY
